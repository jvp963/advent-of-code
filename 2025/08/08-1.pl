...
my @circuits_sorted_largest_to_smallest = sort {$#{$b} <=> $#{$a}} @{$circuits};
my $multipled = 1;
my $circuits_left = 3;
foreach my $circuit (@circuits_sorted_largest_to_smallest) {
	my $size = scalar @{$circuit};
	$multipled *= scalar @{$circuit};
	last unless --$circuits_left;
}

print "Multipled length of 3 longest circuits: $multipled\n";